...
import com.dot2dotz.app.Activities.CustomGooglePlacesSearch;
import com.dot2dotz.app.Activities.HistoryActivity;
import com.dot2dotz.app.Activities.LocationAndGoodsActivity;
import com.dot2dotz.app.Activities.ShowProfile;
import com.dot2dotz.app.Adapter.StatusAdapter;
...
import com.dot2dotz.app.Helper.DataParser;
...
import com.dot2dotz.app.Models.FeedBack;
...
import com.dot2dotz.app.Utils.MapAnimator;
...
public class HomeFragment extends Fragment implements OnMapReadyCallback, LocationListener,
    ...
    MyBoldTextView lblPriceMin, lblBasePricePopup, lblCapacity, lblServiceName, lblPriceKm, lblCalculationType, lblProviderDesc, lblServiceDetails;
    ...
    TextView canel_space_txt;
    LinearLayout lnrInvoice;
    MyBoldTextView lblBasePrice, lblDistanceCovered, lblExtraPrice, lblTimeTaken, lblDistancePrice, waiting_fare, lblCommision, lblTaxPrice, lblTotalPrice, lblPaymentTypeInvoice, lblPaymentChangeInvoice, lblDiscountPrice, lblWalletPrice;
    ...
    private String sourceTrackOrderAddress;
    private JSONArray previousDrop = new JSONArray();
...
//    MapRipple mapRipple;
    ...
    public View onCreateView(LayoutInflater inflater, final ViewGroup container,
        ...
        try {
            ...
            CommonUtils.setLanguage(getActivity());
...
//        <!-- Map frame -->
        ...
        sourceAddressText = rootView.findViewById(R.id.sourceAddressText);
...
//        <!--  Driver and service type Details-->
        ...
        lblServiceDetails = rootView.findViewById(R.id.lblServiceDetails);
...
//          <!--4. Driver Accepted ...-->
        ...
        canel_space_txt = rootView.findViewById(R.id.canel_space_txt);
...
//           <!--5. Invoice Layout ...-->
        ...
        waiting_fare = rootView.findViewById(R.id.waiting_fare);
        ...
        otp_layout = rootView.findViewById(R.id.otp_layout);
        otp_txt = rootView.findViewById(R.id.otp_txt);
...
//                showreasonDialog();
        ...
        cancelRideDialog.setOnShowListener(new DialogInterface.OnShowListener() {
            @Override
            public void onShow(DialogInterface arg) {
                cancelRideDialog.getButton(AlertDialog.BUTTON_POSITIVE).setTextColor(ContextCompat.getColor(context, R.color.colorAccent));
                cancelRideDialog.getButton(AlertDialog.BUTTON_NEGATIVE).setTextColor(ContextCompat.getColor(context, R.color.colorAccent));
            }
        });
    ...
    private void showreasonDialog() {
                                          ...
                                          public void onCheckedChanged(RadioGroup group, int checkedId) {
                                              ...
                                              for (int i = 0; i < feedback_array.size(); i++) {
                                                  if (reason[0].equalsIgnoreCase(feedback_array.get(i).getReason())) {
                                                      cancalReason = feedback_array.get(i).getId();
                                                  }
        ...
        submitBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (cancalReason == -1) {
                    utils.displayMessage(getView(), context, getString(R.string.give_your_feedback));
                } else {
                    cancelRequest("");
                }
    ...
    void layoutChanges() {
        try {
            ...
            if (lnrApproximate.getVisibility() == View.VISIBLE) {//n
                lnrApproximate.startAnimation(slide_down);
            } else if (ScheduleLayout.getVisibility() == View.VISIBLE) {//n
                ScheduleLayout.startAnimation(slide_down);
            } else if (lnrRequestProviders.getVisibility() == View.VISIBLE) {//e
...
//            mMap.setMyLocationEnabled(true);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_PAYMENT && resultCode == Activity.RESULT_OK && data != null) {
            ...
            if (transactionResponse != null && transactionResponse.getPayuResponse() != null) {
                ...
                if (transactionResponse.getTransactionStatus().equals(TransactionResponse.TransactionStatus.SUCCESSFUL)) {
                    //Success Transaction
                    sendStatus(URLHelper.PAYUMONEY_SUCCESS, payuResponse);
                    PreviousStatus = "";
                } else {
                    sendStatus(URLHelper.PAYUMONEY_FAILURE, payuResponse);
                    //Failure Transaction
                }
                            ...
                            if (is_track.equalsIgnoreCase("YES") && CurrentStatus.equalsIgnoreCase("PICKEDUP")) {
                                ...
                                if (!placePredictions.getdAddress().equalsIgnoreCase("")) {
                                    try {
                                        ...
                                        dest_address = Utilities.getAddressUsingLatLng("destination", frmDest, context, "" + dest_lat,
                                                "" + dest_lng);
                                        if (frmDest != null && dest_address != null)
                                        ...
                                        if (!dest_lat.equalsIgnoreCase("") && !dest_lng.equalsIgnoreCase("")) {
                                            try {
                                                ...
                                                int padding = 270; // offset from edges of the map in pixels
...
//            JSONObject jsonResult = jsonObject.optJSONObject("result");
            json.put("mihpayid", "7438825831");
            json.put("txnid", "146");
            json.put("amount", "10.00");
            json.put("error_Message", "No Error");
            json.put("status", "0");
            json.put("udf1", "602");
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, URLHelper.PAYUMONEY_SUCCESS, json, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                ...
                Toast.makeText(context, getResources().getString(R.string.payment_done), Toast.LENGTH_LONG).show();
            }
                        ...
                        try {
                            ...
                            if (response.statusCode == 400 || response.statusCode == 405 || response.statusCode == 500) {
                                ...
                                if (json != null && !json.equalsIgnoreCase("")) {
                                    ...
                                    displayMessage(getResources().getString(R.string.please_try_again));
                                }
    ...
    private void sendStatus(final String strURL, final String payUResponse) {
        customDialog = new CustomDialog(context);
        if (customDialog != null)
            customDialog.show();
        JSONObject jsonObject;
        ...
        try {
            ...
            JSONObject jsonResult = jsonObject.optJSONObject("result");
            json.put("mihpayid", jsonResult.optString("mihpayid"));
            json.put("txnid", jsonResult.optString("txnid"));
            json.put("amount", jsonResult.optString("amount"));
            json.put("error_Message", jsonResult.optString("error_Message"));
            json.put("status", jsonObject.optString("status") + "");
            json.put("udf1", jsonResult.optString("udf1"));
...
//                Log.e("MyTestError1", "" + response.statusCode);
                    if (response != null && response.data != null) {
                        ...
                        if (error instanceof NoConnectionError) {
                            ...
                            sendStatus(strURL, payUResponse);
                        }
                    }
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    private void sendStatu(String strURL, String payUResponse) {
        if (customDialog != null)
            customDialog.show();
        ...
        try {
            ...
            json.addProperty("udf1", jsonResult.optString("udf1"));
        ...
        Ion.with(getContext())
                ...
                .addHeader("Authorization", "Bearer " + SharedHelper.getKey(context, "access_token"))
                    ...
                    public void onCompleted(Exception e, com.koushikdutta.ion.Response<String> response) {
                        ...
                        if (e != null) {
                            if (e instanceof TimeoutException) {
                                Toast.makeText(context, context.getResources().getString(R.string.please_try_again), Toast.LENGTH_SHORT);
                            }
                            return;
                        }
    ...
    /*void showProviderPopup(JSONObject jsonObject) {
        ...
        lblServiceDetails.setText("" + jsonObject.optString("weight"));
...
*/
    ...
    public void getServiceList() {
        ...
        try {
            ...
            Log.e("URL: ", URLHelper.GET_SERVICE_LIST_API);
...
//            }
    ...
    public void sendRequest() {
        ...
        Log.e("URL: ", URLHelper.SEND_REQUEST_API);
                        ...
                        case 200:
                            ...
                            try {
                                ...
                                if(jsonObject.has("message"))
                                {
                                    ...
                                    if(message.equals("No Drivers Found"))
                                    {
                                        Toast.makeText(getActivity(),"currently no driver found of this vehicle",Toast.LENGTH_SHORT).show();
                                    }
                                }
                            ...
                            Log.e("check","Umit Responses:"+json);
                            break;
                    }
            ...
            public Map<String, String> getParams() throws AuthFailureError {
                ...
                try {
                    for (int i = 1; i <= location_array.size(); i++) {
                        ...
                        params.put("user_name[" + i + "]", location_array.get(i - 1).getUserName());
                        params.put("user_mobile[" + i + "]", location_array.get(i - 1).getUserMobile());
                    }
                    try {
    ...
    public void cancelRequest(String anotherService) {
        customDialog = new CustomDialog(context);
        customDialog.setCancelable(false);
        if (customDialog != null)
            customDialog.show();
        JSONObject object = new JSONObject();
        if (anotherService.equalsIgnoreCase("Need Another Service")) {
            try {
                object.put("request_id", SharedHelper.getKey(context, "request_id"));
                object.put("cancel_reason", anotherService);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
...
//                    destination.setText(SharedHelper.getKey(context, "extend_address"));
    ...
    private void showChooser() {
        Intent intent = new Intent(getActivity(), com.dot2dotz.app.Activities.Payment.class);
        intent.putExtra("PaymentMode", lblPaymentType.getText().toString());
    ...
    private void getCardDetailsForPayment(CardInfo cardInfo) {
        ...
        } else if (cardInfo.getLastFour().equals("PAYUMONEY")) {
            SharedHelper.putKey(context, "card_id", cardInfo.getCardId());
            SharedHelper.putKey(context, "payment_mode", "PAYUMONEY");
            imgPaymentType.setImageResource(R.drawable.pay_u_money);
            lblPaymentType.setText("PayU money");
        } else {
            SharedHelper.putKey(context, "card_id", cardInfo.getCardId());
    ...
    private void startPayUMoneyTransation(JSONObject response) {
        ...
        double amount = 0;
        try {
            amount = Double.parseDouble("" + response.optString("amount"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        String productName = response.optString("productinfo");
        String firstName = response.optString("firstName");
        String email = response.optString("email");
        String udf1 = SharedHelper.getKey(context, "request_id") + "";
        String udf2 = "";
        String udf3 = "";
        String udf4 = "";
        String udf5 = "";
        ...
        builder.setAmount(response.optDouble("amount"))
                .setTxnId(txnId)
                .setPhone(phone)
                .setProductName(productName)
                .setFirstName(firstName)
                .setEmail(email)
                .setsUrl(response.optString("surl"))
                .setfUrl(response.optString("furl"))
                .setUdf1(udf1)
                .setUdf2(udf2)
                .setUdf3(udf3)
                .setUdf4(udf4)
                ...
                .setUdf10(udf10)
                .setIsDebug(false)
                .setKey(response.optString("KEY"))
                .setMerchantId(response.optString("merchantSalt"));

        mPaymentParams = builder.build();

        mPaymentParams.setMerchantHash(response.optString("hash"));

        PayUmoneyFlowManager.startPayUMoneyFlow(mPaymentParams, getActivity(), R.style.AppTheme_default, true);
    }

    private void payNowPayUmoney() {
        ...
        Log.e("URL: ", URLHelper.PAY_NOW_API);
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, URLHelper.PAY_NOW_API, object, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                ...
                startPayUMoneyTransation(response);
        ...
        try {
            if (!isPopupVisible) {
                ...
                if (isInternet) {
                    ...
                    Log.e("URL: ", URLHelper.REQUEST_STATUS_CHECK_API);
                    final JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET,
                        ...
                        public void onResponse(JSONObject response) {
                            ...
                            try {
                                providerTrip = response.optString("Providertrip");

                            /*if (!providerTrip.equalsIgnoreCase(null) && !providerTrip.equalsIgnoreCase("")) {
                                if (!showTimeDialog(providerTrip).isShowing()) {
                                    showTimeDialog(providerTrip).show();
                                }
                            }*/

                                userdrop = response.getJSONArray("userdrop");
                                if (userdrop != null && !userdrop.toString().equalsIgnoreCase(previousDrop.toString())) {
                                    previousDrop = userdrop;
                                    statusArrayList = new ArrayList<>();
                                    sourceTrackOrderAddress = userdrop.getJSONObject(0).optString("s_address");
                                    for (int i = 0; i < userdrop.length(); i++) {
                                        TripStatus flows = new TripStatus();
//                                    sourceTrackOrderAddress = userdrop.getJSONObject(i).optString("s_address");
                                        ...
                                        if (userdrop.getJSONObject(i).optString("status").equalsIgnoreCase("STARTED")) {
                                            Log.e("mycheck","started otp");
                                            otp_txt.setText(userdrop.getJSONObject(i).optString("arrived_otp"));
                                            current_otp = userdrop.getJSONObject(i).optString("arrived_otp");
                                        }
                                    }
                                    setupRecyclerView();
                            ...
                            if (response.optJSONArray("data") != null && response.optJSONArray("data").length() > 0) {
                                ...
                                try {
                                    ...
                                    assignedAt = requestStatusCheckObject.optString("assigned_at");
                                    Log.e(TAG, "requestStatusCheckObject: assignedAt " + assignedAt);
                                    //Driver Detail
                                    ...
                                    if (!PreviousStatus.equals(status)) {
                                        ...
                                        switch (status) {
                                            ...
                                            long diffTime = CommonUtils.getCurrentAndAssignedTime(assignedAt,currentTime);*/
                                            ...
                                            case "ACCEPTED":
                                                ...
                                                try {
                                                    ...
                                                    Log.e("mycheck","compled otp");
                                                    otp_txt.setText(requestStatusCheckObject.optString("arrived_otp"));
                                                    ...
                                                    canel_space_txt.setVisibility(View.VISIBLE);
                                            ...
                                            case "ARRIVED":
                                                ...
                                                try {
                                                    ...
                                                    btnCancelTrip.setText(context.getResources().getString(R.string.cancel_trip));
                                                    canel_space_txt.setVisibility(View.GONE);
                                            ...
                                            case "DROPPED":
                                                ...
                                                try {
                                                    ...
                                                    /*if(payment.optInt("discount") != 0) {
                                                        discountLayout.setVisibility(View.VISIBLE);
                                                        discount.setText(SharedHelper.getKey(context, "currency") + " " + payment.optString("discount"));
                                                    } else {
                                                        discountLayout.setVisibility(View.GONE);
                                                    }*/
                                                        lblDistanceCovered.setText(requestStatusCheckObject.optString("distance") + " mi");

                                                        Float estimatedfare = Float.valueOf(payment.optString("fixed")) + Float.valueOf(payment.optString("distance"));
...
//                                                    lblBasePrice.setText(SharedHelper.getKey(context, "currency") + "" + payment.optString("fixed"));
                                                        ...
                                                        waiting_fare.setText(SharedHelper.getKey(context, "currency") + "" + payment.optString("waiting_charge"));
                                                        ...
                                                        if (isPaid.equalsIgnoreCase("1") && paymentMode.equalsIgnoreCase("CASH")
                                                        ...
                                                        } else if (isPaid.equalsIgnoreCase("1") && paymentMode.equalsIgnoreCase("PAYUMONEY")
                                                                && walletAmountDetected > 0 && couponAmountDetected > 0 && totalRideAmount == 0) {
                                                            btnPayNow.setVisibility(View.GONE);
                                                            btnPaymentDoneBtn.setVisibility(View.VISIBLE);
                                                            walletDetectionLayout.setVisibility(View.VISIBLE);
                                                            ...
                                                            flowValue = 5;
                                                            layoutChanges();
                                                            imgPaymentTypeInvoice.setImageResource(R.drawable.pay_u_money);
                                                            lblPaymentTypeInvoice.setText("PAYUMONEY");
                                                            ...
                                                            imgPaymentTypeInvoice.setImageResource(R.drawable.visa);
                                                            lblPaymentTypeInvoice.setText("CARD");
                                                        } else if (isPaid.equalsIgnoreCase("0") && paymentMode.equalsIgnoreCase("PAYUMONEY")
                                                                && walletAmountDetected > 0 && couponAmountDetected > 0 && totalRideAmount > 0) {
                                                            btnPayNow.setVisibility(View.VISIBLE);
                                                            btnPaymentDoneBtn.setVisibility(View.GONE);
                                                            walletDetectionLayout.setVisibility(View.VISIBLE);
                                                            ...
                                                            imgPaymentTypeInvoice.setImageResource(R.drawable.visa);
                                                            lblPaymentTypeInvoice.setText("CARD");
                                                        } else if (isPaid.equalsIgnoreCase("1") && paymentMode.equalsIgnoreCase("PAYUMONEY") && walletAmountDetected > 0 && couponAmountDetected == 0 && totalRideAmount == 0) {
                                                            btnPayNow.setVisibility(View.GONE);
                                                            btnPaymentDoneBtn.setVisibility(View.VISIBLE);
                                                            walletDetectionLayout.setVisibility(View.VISIBLE);
                                                            discountDetectionLayout.setVisibility(View.GONE);
                                                            ...
                                                            imgPaymentTypeInvoice.setImageResource(R.drawable.visa);
                                                            lblPaymentTypeInvoice.setText("CARD");
                                                        } else if (isPaid.equalsIgnoreCase("0") && paymentMode.equalsIgnoreCase("PAYUMONEY") && walletAmountDetected == 0 && couponAmountDetected > 0 && totalRideAmount > 0) {
                                                            btnPayNow.setVisibility(View.VISIBLE);
                                                            btnPaymentDoneBtn.setVisibility(View.GONE);
                                                            walletDetectionLayout.setVisibility(View.GONE);
                                                            discountDetectionLayout.setVisibility(View.VISIBLE);
                                            ...
                                            case "COMPLETED":
                                                ...
                                                try {
                                                    ...
                                                    /*if(payment.optInt("discount") != 0) {
                                                        ...
                                                        lblDistancePrice.setText(SharedHelper.getKey(context, "currency") + ""
                                                                + payment.optString("distance"));
                                                        waiting_fare.setText(SharedHelper.getKey(context, "currency") + ""
                                                                + payment.optString("waiting_charge"));
                                ...
                                try {
                                    ...
                                    if (scheduledDate != null && scheduledTime != null && !scheduledDate.equalsIgnoreCase("")
                                        ...
                                        //Toast.makeText(context, context.getResources().getString(R.string.no_providers_found), Toast.LENGTH_SHORT).show();

                                        android.support.v7.app.AlertDialog.Builder builder = new android.support.v7.app.AlertDialog.Builder(context);
                                        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
                                        builder.setMessage(context.getResources().getString(R.string.no_providers_found))
    ...
    private Dialog showTimeDialog(String providerTrip) {
        dialog = new Dialog(getContext());
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        dialog.setContentView(R.layout.activity_time_dialog);
        dialog.setCancelable(false);
        dialog.setCanceledOnTouchOutside(false);
        TextView mProviderTrip = dialog.findViewById(R.id.providerTrip);
        Button mNewRide = dialog.findViewById(R.id.newRide);
        mProviderTrip.setText(getString(R.string.no_driver));
        mNewRide.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                dialog.dismiss();
                cancelRequest("Need Another Service");
            }
        });
...
//	        }
    ...
    public void submitReviewCall() {
        ...
        Log.e("URL: ", URLHelper.RATE_PROVIDER_API);
...
//        }
    ...
    public void onTrackbtn(TripStatus statusflow) {
        ...
        fetchUrl = new FetchUrl();
        fetchUrl.execute(url);
        otp_txt.setText(statusflow.getOtp());
        //  LatLng location = new LatLng(Double.parseDouble(current_lat), Double.parseDouble(current_lng));
    }
    ...
    public void postSumUpPayment(int resultCode, Intent data) {
    ...
    class OnClick implements View.OnClickListener {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.backArrow:
                    status.setVisibility(View.GONE);
//                    otp_txt.setText(current_otp);
                    break;

                case R.id.status_info:
                    status.setVisibility(View.VISIBLE);
                    if (sourceTrackOrderAddress != null && !sourceTrackOrderAddress.isEmpty()) {
                        sourceAddressText.setText(sourceTrackOrderAddress);
                    } else {
                ...
                case R.id.scheduleBtn:
                    ...
                    if (scheduledDate != "" && scheduledTime != "")
                    {
                        ...
                        if (!DateUtils.isToday(milliseconds)) {
                            ...
                            {
                                Toast.makeText(getContext(),"plz choose date and time",Toast.LENGTH_SHORT).show();
                                return;
                            }
                        } else {
                        ...
                        public void onTimeSet(TimePicker timePicker, int selectedHour, int selectedMinute) {
                            ...
                            if (callCount == 0) {
                                ...
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                                    if (!Objects.equals(scheduledDate, "") &&
                                            !Objects.equals(scheduledTime, "")) {
                                        @SuppressLint("SimpleDateFormat")
                                        SimpleDateFormat sdf = new SimpleDateFormat("dd");
                                        Date now = new Date();
                                        String strTime = sdf.format(now);
                                        if (choosedDate.equalsIgnoreCase(strTime)) {
                                            Date date = null;
                                            try {
                                                date = new SimpleDateFormat("dd-MM-yyyy", Locale.ENGLISH).parse(scheduledDate);
                                            } catch (ParseException e) {
    ...
    private class ServiceListAdapter extends RecyclerView.Adapter<ServiceListAdapter.MyViewHolder> {
        ...
        public void onBindViewHolder(ServiceListAdapter.MyViewHolder holder, final int position) {
            ...
            holder.serviceItemDetails.setText(jsonArray.optJSONObject(position).optString("weight"));

            if (position == currentPostion) {
                SharedHelper.putKey(context, "service_type", "" + jsonArray.optJSONObject(position).optString("id"));
                SharedHelper.putKey(context, "name", "" + jsonArray.optJSONObject(position).optString("name"));
                Glide.with(activity).load(jsonArray.optJSONObject(position).optString("image")).skipMemoryCache(false)
        ...
        public class MyViewHolder extends RecyclerView.ViewHolder {
            ...
            public MyViewHolder(View itemView) {
                ...
                serviceItemDetails = (MyTextView) itemView.findViewById(R.id.serviceItemDetails);
